...
#### Что реализовано (на максимальные 10 / 10 баллов):
* **Подключение к онлайн-сервисам, IP-геолокация и curs.md (пункт a, 5 баллов):** 
  * Автоматическое определение **публичного IP-адреса, провайдера (ISP), города и точных координат** через `ip-api.com`.
  * Запрос текущей погоды в реальном времени через **Open-Meteo API** по реальным координатам пользователя.
  * Интеграция с ведущим финансовым порталом Молдовы **[curs.md](https://www.curs.md/ro)**: вывод официального курса **BNM (Banca Națională a Moldovei)** и реальных рыночных котировок коммерческих банков Молдовы (**покупка / продажа** для **EUR**, **USD**, **RON** к лею MDL).
